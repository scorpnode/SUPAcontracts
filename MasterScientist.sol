...
// File: @openzeppelin/contracts@3.4.1/utils/EnumerableSet.sol
...
// File: @openzeppelin/contracts@3.4.1/utils/Address.sol
...
// File: @openzeppelin/contracts@3.4.1/token/ERC20/ERC20.sol
...
//todo: burn, transfer rather than mint, token locking, claim/transfer, change blocks rewards+time+rewards

pragma solidity 0.6.12;
interface Vesting { 
function lock(address _holder, uint256 _amount) external returns(bool);
}



// MasterScientist is the master scientist of whatever studies are available.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once SUPA is sufficiently
...
contract MasterScientist is Ownable, Authorizable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SUPA to distribute per block.
        uint256 lastRewardBlock; // Last block number that SUPA distribution occurs.
        uint256 accGovTokenPerShare; // Accumulated SUPA per share, times 1e12. See below.
        IERC20 partnerToken; //address of partner token contract.
        uint256 partnerRewardsStartTime;
        uint256 partnerRewardsEndTime;
        uint256 partnerTokensAvailable;
        uint256 lastRewardPartnerTime;
   ...
   IERC20 public immutable govToken;
    ...
    // Founder Reward
    // SUPA token per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus multiplier for early SUPA makers.
    ...
    // The block number when SUPA mining starts.
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate,IERC20 _partnerToken) public onlyOwner nonDuplicated(_lpToken) {
        ...
        poolInfo.push(
            PoolInfo({
                ...
                partnerToken:_partnerToken, //address of partner token contract.
                partnerRewardsStartTime:0,
                 partnerRewardsEndTime:0,
                partnerTokensAvailable:0,
                 lastRewardPartnerTime:0,
                 accPartnerTokenPerShare:0
            })
        );
    }
     function updatePartnerRewards(uint _pid, uint _startTime, uint _endTime, IERC20 _partnerToken, bool _toUpdate) public onlyOwner {
        ...
        pool.partnerRewardsStartTime=_startTime;
        pool.partnerRewardsEndTime=_endTime;
       pool.partnerToken=_partnerToken;
       pool.partnerTokensAvailable=pool.partnerToken.balanceOf(address(this));
        if(_toUpdate==true){
    ...
    // Update the given pool's SUPA allocation points. Can only be called by the owner.
    ...
    function updatePoolPartner(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        //check to see if recently allocated tokens for pool. checks to see if reward for partner is active
        if (block.timestamp <= pool.lastRewardPartnerTime || pool.partnerRewardsStartTime==0 || pool.partnerRewardsEndTime==0) {
            return;
        }

        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
          uint256 partnerSupply= pool.partnerToken.balanceOf(address(this));
           if(pool.partnerTokensAvailable> partnerSupply){
               pool.partnerTokensAvailable=partnerSupply; 
            }
//checks to see if rewards for partner has started or ended
        if (pool.partnerTokensAvailable == 0 || block.timestamp<pool.partnerRewardsStartTime || block.timestamp>=pool.partnerRewardsEndTime){
            return;
        }
        if (lpSupply == 0) {
            pool.lastRewardPartnerTime = block.timestamp;
            return;
        }

        uint256 PartnerTokenForFarmer =getPoolPartnerReward(pool.lastRewardPartnerTime,block.timestamp,_pid);
       //adds allocation per LP share
        pool.accPartnerTokenPerShare=pool.accPartnerTokenPerShare.add(
           PartnerTokenForFarmer.mul(1e12).div(lpSupply)


        );
        
        pool.partnerTokensAvailable= pool.partnerTokensAvailable.sub(PartnerTokenForFarmer);
        pool.lastRewardPartnerTime = block.timestamp;
      
    
    
    }
    function getPoolPartnerReward(uint previousTime, uint currentTime, uint _pid) public
        view
        returns (
            uint256 forFarmer  
        ){
            ...
            if(previousTime<pool.partnerRewardsStartTime){
                    previousTime=pool.partnerRewardsStartTime;
            }
            uint256 timeRemaining=pool.partnerRewardsEndTime.sub(currentTime);
            //rewards per second
        ...
        )
    {
        ...
        uint256 GovernanceTokenCanMint = govToken.balanceOf(vaultaddr);
    ...
    // View function to see pending SUPA on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
     ...
     function pendingRewardPartner(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPartnerTokenPerShare = pool.accPartnerTokenPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
         
        if (block.timestamp > pool.lastRewardPartnerTime && lpSupply > 0 && pool.partnerTokensAvailable >0 && block.timestamp>=pool.partnerRewardsStartTime && block.timestamp<pool.partnerRewardsEndTime && pool.partnerRewardsStartTime>0 && pool.partnerRewardsEndTime>0) {
        uint256 PartnerTokenForFarmer =getPoolPartnerReward(pool.lastRewardPartnerTime,block.timestamp,_pid);
        accPartnerTokenPerShare=pool.accPartnerTokenPerShare.add(
           PartnerTokenForFarmer.mul(1e12).div(lpSupply)


        );
        }
        return user.amount.mul(accPartnerTokenPerShare).div(1e12).sub(user.rewardDebtPartner);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 pendingPartner =
                user.amount.mul(pool.accPartnerTokenPerShare).div(1e12).sub(
            ...
            if (pendingPartner >partnerBal) {
                pendingPartner = partnerBal;
            }
            if (pending > 0) {
...
10                user.blockdelta = block.number - user.lastWithdrawBlock;
            ...
            {
                //25% fee if a user deposits and withdraws in between same block and 59 minutes.
                ...
                //8% fee if a user deposits and withdraws after 1 hour but before 1 day.
                pool.lpToken.safeTransfer(
                    address(msg.sender),
                ...
                //4% fee if a user deposits and withdraws between after 1 day but before 3 days.
                pool.lpToken.safeTransfer(
                    address(msg.sender),
                ...
                //2% fee if a user deposits and withdraws after 3 days but before 5 days.
                pool.lpToken.safeTransfer(
                    address(msg.sender),
                ...
                //1% fee if a user deposits and withdraws if the user withdraws after 5 days but before 2 weeks.
                pool.lpToken.safeTransfer(
                    address(msg.sender),
                ...
                //0.5% fee if a user deposits and withdraws after 2 weeks.
                pool.lpToken.safeTransfer(
                    address(msg.sender),
            ...
            } else if (user.blockdelta > blockDeltaStartStage[6]) {
                //0.05% fee if a user deposits and withdraws after 4 weeks.
                pool.lpToken.safeTransfer(
                    address(msg.sender),
    ...
    // Safe GovToken transfer function, just in case if rounding error causes pool to not have enough GovTokens.
     function safeGovTokenTransfer(address _to, uint256 _amount) onlyOwner public {
      ...
      function safePartnerTokenTransfer(address _to, uint256 _amount,IERC20 tokenAddr) onlyOwner public{
        ...
        bool transferSuccess = false;
        if (_amount > partnerTokenBal) {
            transferSuccess = tokenAddr.transfer(_to, partnerTokenBal);
        } else {
    ...
    function vault(address _vaultaddr) public onlyAuthorized {
        vaultaddr = _vaultaddr;
    }
   ...
   function userDelta(uint256 _pid) public view returns (uint256) {
    ...
    function setUserDepFee(uint256 _usrDepFees) public onlyAuthorized() {
     ...
     function setVestingddress(address _vesting)  public onlyAuthorized(){
       VestingContractAddr=_vesting;
    }
}
